...
// A compressed copy of one take stem, for browser playback only. The 24-bit source is
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${AGENT_API_URL}/api/produce/sessions/stems/${stemId}/preview`,
      { method: 'GET', headers }
    );
    ...
    console.error('Session stems preview error:', error);
    return NextResponse.json(
