...
| 2026-09-19 | Auth & access | [memory/auth_access.md](memory/auth_access.md) | Editor invites as a copyable link (no mail stack here): hash-only token storage, single-use enforced in the `WHERE` clause, email-bound so a forwarded link is worthless, and how the token crosses the OAuth round-trip. Plus HMAC-signing the `appSession` cookie (it was unsigned JSON anyone could write), the two unguarded `/api/users` routes, and `requireAuth` failing open. |
